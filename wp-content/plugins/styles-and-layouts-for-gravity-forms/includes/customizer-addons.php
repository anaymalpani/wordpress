...
class Woocommerce_Addon_Custom_Control extends WP_Customize_Control
{
  /**
   * Render the control's content.
   * Allows the content to be overriden without having to rewrite the wrapper.
  ...
  public function render_content() {
    ...
    <label>
        <h2><?php echo esc_html( $this->label ); ?></h2>
        <a href="https://wpmonks.com/downloads/woocommerce-addon?src=customizer" target="_blank"><img src="<?php echo GF_STLA_URL; ?>/css/images/woocommerce-addon.jpg"></a>
        <h3>Save styles for forms added using Gravity Forms Product Add-ons WooCommerce plugin</h3>
        <hr>
      </textarea>
    </label>
    <?php
...
$wp_customize->add_control(
  new Widget_Sidebar_Custom_Control(
    $wp_customize, // WP_Customize_Manager
    'gf_stla_form_id_[addons][widget-sidebar]', // Setting id
    array( // Args, including any custom ones.
...
$wp_customize->add_control(
  new Woocommerce_Addon_Custom_Control(
    $wp_customize, // WP_Customize_Manager
    'gf_stla_form_id_[addons][woocommerce-addon]', // Setting id
    array( // Args, including any custom ones.
     'label' => __( 'WooCommerce Addon' ),
      'section' => 'gf_stla_form_id_addons',
    )
  )
);
